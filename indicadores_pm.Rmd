...
```{r read_arrecadacao_propria}
df <- readxl::read_excel(ds_file, sheet = "ArrecadacaoPropria", skip = 2)
df$mes <- as.factor(df$mes)
# df
```

```{r table_arrecadacao_propria_mensal}
data <- select(df, mes, arrec_mes_ant, arrec_mes_atual, prev_mes_atual)
colnames(data) <- c("Mês", paste("Arrecadado em", ano_anterior), paste("Arrecadado em", ano_base), paste("Previsto para", ano_base))
...
```{r table_arrecadacao_propria_acum}
data <- select(df, mes, arrec_acum_ant, arrec_acum_atual, prev_acum_atual)
colnames(data) <- c("Mês", paste("Arrecadado em", ano_anterior), paste("Arrecadado em", ano_base), paste("Previsto para", ano_base))
...
```{r table_arrecadacao_propria_excesso}
data <- select(df, mes, vl_var_ant, vl_var_prev,	var_ant,	var_prev)
data$var_ant <- percent(data$var_ant, big.mark = ".", decimal.mark = ",", accuracy = 0.01)
data$var_prev <- percent(data$var_prev, big.mark = ".", decimal.mark = ",", accuracy = 0.01)
colnames(data) <- c("Mês", paste(ano_base, "(-)", ano_anterior), paste("Arrecadado (-) Previsto", ano_base), paste(ano_base, "/", ano_anterior, "(%)"), paste("Arrecadado / Previsto", ano_base, "(%)"))
...
```{r plot_arrecadacao_propria_mensal}
data <- select(df, mes, arrec_mes_ant, arrec_mes_atual, prev_mes_atual)
colnames(data) <- c("Mês", paste("Arrecadado em", ano_anterior), paste("Arrecadado em", ano_base), paste("Previsto para", ano_base))
data <- gather(data, tipo, valor, c(paste("Arrecadado em", ano_anterior), paste("Arrecadado em", ano_base), paste("Previsto para", ano_base)))
data$tipo <- as.factor(data$tipo)
...
```{r plot_arrecadacao_propria_acum}
data <- select(df, mes, arrec_acum_ant, arrec_acum_atual, prev_acum_atual)
colnames(data) <- c("Mês", paste("Arrecadado em", ano_anterior), paste("Arrecadado em", ano_base), paste("Previsto para", ano_base))
data <- gather(data, tipo, valor, c(paste("Arrecadado em", ano_anterior), paste("Arrecadado em", ano_base), paste("Previsto para", ano_base)))
data$tipo <- as.factor(data$tipo)
...
```{r table_transf_corr_fed_excesso}
data <- select(df, mes, vl_var_ant, vl_var_prev,	var_ant,	var_prev)
data$var_ant <- percent(data$var_ant, big.mark = ".", decimal.mark = ",", accuracy = 0.01)
data$var_prev <- percent(data$var_prev, big.mark = ".", decimal.mark = ",", accuracy = 0.01)
colnames(data) <- c("Mês", paste(ano_base, "(-)", ano_anterior), paste("Arrecadado (-) Previsto", ano_base), paste(ano_base, "/", ano_anterior, "(%)"), paste("Arrecadado / Previsto", ano_base, "(%)"))
...
```{r table_transf_corr_est_excesso}
data <- select(df, mes, vl_var_ant, vl_var_prev,	var_ant,	var_prev)
data$var_ant <- percent(data$var_ant, big.mark = ".", decimal.mark = ",", accuracy = 0.01)
data$var_prev <- percent(data$var_prev, big.mark = ".", decimal.mark = ",", accuracy = 0.01)
colnames(data) <- c("Mês", paste(ano_base, "(-)", ano_anterior), paste("Arrecadado (-) Previsto", ano_base), paste(ano_base, "/", ano_anterior, "(%)"), paste("Arrecadado / Previsto", ano_base, "(%)"))
...
O serviço de contabilidade limita-se à sua função primária, que é a de fornecer informações confiáveis, relevantes e tempestivas com vistas a apoiar o processo de tomada de decisão e à prestação de contas, atendendo ao princípio da transparência.

Cabe à gestão da entidade, primariamente, a avaliação da gestão bem como identificais oportunidades e ameaças e, por esse motivo, é que este relatório se abstém de formar qualquer juízo a respeito da eficiência, eficácia e efetividade da gestão, tarefa que derradeiramente cabe aos órgãos de controle e, ao Legislativo e à sociedade como um todo.

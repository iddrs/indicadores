...
pdflatex -file-line-error -halt-on-error -output-directory cache/ -output-format pdf pm_report.tex
# Executa duas vezes para formar o sumário
pdflatex -file-line-error -halt-on-error -output-directory cache/ -output-format pdf pm_report.tex

# Renomeando a saída para output
Move-Item -Path cache\pm_report.pdf -Destination $Output -Force
